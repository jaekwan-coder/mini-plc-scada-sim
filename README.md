...
**🔴 [라이브 데모 보기](https://github.com/jaekwan-coder/mini-plc-scada-sim.git/)** — 다운로드나 설치 없이,
브라우저에서 바로 컨베이어/센서/실린더 동작을 눈으로 볼 수 있습니다. (`docs/index.html`,
GitHub Pages로 호스팅. 링크는 저장소 이름에 맞게 아래 9번 "라이브 데모 켜기" 항목 참고)
...
`docs/index.html`은 `plc_logic.py`의 상태머신 로직을 웹 애니메이션으로 그대로 옮겨놓은
독립 실행형 페이지입니다. GitHub에 올리고 아래처럼 켜기만 하면, 링크 하나로 누구나
설치 없이 브라우저에서 바로 동작을 볼 수 있습니다.

1. 이 프로젝트 전체를 GitHub 저장소로 올립니다 (`docs` 폴더 포함해서 그대로).
2. 저장소 페이지에서 **Settings → Pages**로 들어갑니다.
3. "Branch" 항목에서 `main` 브랜치, 폴더는 `/docs`를 선택하고 저장합니다.
4. 1~2분 기다리면 `https://내깃허브아이디.github.io/저장소이름/` 주소가 활성화됩니다.
5. 이 README 맨 위 "라이브 데모 보기" 링크의 `YOUR-GITHUB-ID`와 `YOUR-REPO-NAME`을
   실제 아이디/저장소 이름으로 바꿔줍니다.
